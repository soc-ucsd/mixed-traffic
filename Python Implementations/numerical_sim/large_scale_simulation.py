...
for i in range(len(AV_array)):
   ...
   # Position of the perturbation
            ...
            if (k > ActuationTime / Tstep):
                if (controllerType == 1):
                    ...
                    for i_AV in range(AV_number):
                        ...
                        v_cmd[k + 1] = beta_temp * (alpha_temp * v_target + (1 - alpha_temp) * S[k, id_AV - 1, 1]) + (
                                    1 - beta_temp) * v_cmd[k]
                ...
                for i_AV in range(0, AV_number):
                    id_AV = AV_position[0][i_AV]
                    flag = (pow(S[k, id_AV, 1], 2) - pow(S[k, id_AV - 1, 1], 2)) / 2 / (
                                S[k, id_AV - 1, 0] - S[k, id_AV, 0] - sd) > abs(dcel_max)
                    if (flag.any()):
                        u[i_AV] = dcel_max
                    S[k, id_AV, 2] = u[i_AV]
    ...
    Maximum_Spacing.append(round(max_space, 2))
    ...
    Control_energy.append(controlEnergy)
